...
class DatabaseWidget(QWidget):
    ...
    def select_directory(self):
        ...
        directory = QFileDialog.getExistingDirectory(
            ...
            | QFileDialog.Option.DontResolveSymlinks,  # opzioni
        )
    ...
    def update_database(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to update the database: {str(e)}"
            )
        else:
            self.logger.info("Database update completed successfully")
            QMessageBox.information(
                self, "Success", "Database has been successfully updated."
            )
    ...
    def create_restore_database(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "Success", f"Database restore point created: {backup_path}"
            )
            logging.info(f"Database backup created: {backup_path}")

        except Exception as e:
            QMessageBox.critical(
                self, "Error", f"Failed to create database restore point: {str(e)}"
            )
